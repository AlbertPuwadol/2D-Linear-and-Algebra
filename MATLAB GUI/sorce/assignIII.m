function varargout = assignIII(varargin)
%ASSIGNIII MATLAB code file for assignIII.fig
%      ASSIGNIII, by itself, creates a new ASSIGNIII or raises the existing
%      singleton*.
%
%      H = ASSIGNIII returns the handle to a new ASSIGNIII or the handle to
%      the existing singleton*.
%
%      ASSIGNIII('Property','Value',...) creates a new ASSIGNIII using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to assignIII_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      ASSIGNIII('CALLBACK') and ASSIGNIII('CALLBACK',hObject,...) call the
%      local function named CALLBACK in ASSIGNIII.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help assignIII

% Last Modified by GUIDE v2.5 09-Dec-2019 17:07:50

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @assignIII_OpeningFcn, ...
                   'gui_OutputFcn',  @assignIII_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before assignIII is made visible.
function assignIII_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for assignIII
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes assignIII wait for user response (see UIRESUME)
% uiwait(handles.figure1);
axes(handles.axes1)
matlabImage = imread('image4.png');
image(matlabImage)
axis off
axis image
...
function varargout = assignIII_OutputFcn(hObject, eventdata, handles)
